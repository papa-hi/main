...
- **Key Features**:
    ...
    - **Availability Notifications** (`server/availability-match-notifications.ts`): Dutch-language email templates (Resend API) and push notifications for: setup confirmation, new match alerts (top 3 per calculation, deduplicated), weekly digest, day-before reminders. Emails use purple-gradient branding.
    - **Availability Cron Jobs** (`server/availability-cron-jobs.ts`): Three scheduled jobs via `node-cron` — nightly match recalculation (2am), weekly digest emails (Sunday 6pm), day-before push reminders (daily 8pm). Rate-limited batch processing with error handling. Initialized in `server/index.ts` on server start.
